...
test("run reads and stored traces apply tool-scoped output restrictions", async () => {
  const lead = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch: request => new URL(request.url).pathname.startsWith("/internal/operations/") ? new Response("Missing", { status: 404 }) : Response.json({ lead_id: "LD-2291", estimated_acv: 95000 }) });
  ...
  try {
    const hook = { execution_id: "verify", context: { user_id: "verify@example.test" }, tool: { toolkit: "Lead", name: "RouteLead", version: "1.0.0" }, inputs: { lead_id: "LD-2291", estimated_acv: 95000, owner_email: "riley@example.test", rationale: "Fit", operation_key: "verify" } };
    await call("/pre", "hook", hook);
    await call("/post", "hook", { ...hook, success: true, output: { personal_phone: "+1-415-555-0137" } });
    ...
    await call("/internal/runs/privacy/result", "web", { status: "completed", text: "Finished", tool_calls: [{ mcpName: "Lead.RouteLead", name: "Lead_RouteLead", args: { rationale: "Enterprise SOURCE_SECRET", lead_id: "LD-2291" } }] });
    const doc = await call("/operator/policy", "operator");
    doc.output_rules.push({ ...doc.output_rules[0], id: "route-only-secret", match: { toolkit: "Lead", tool: "RouteLead" }, fields: [], patterns: [{ id: "secret", regex: "SOURCE_SECRET", strategy: "remove" }] });
    ...
    expect(shown.run.tool_calls[0].args.lead_id).toBe("LD-2291");
    ...
    const stored = await call("/internal/runs/privacy-write/result", "web", { status: "completed", text: "SOURCE_SECRET", tool_calls: [{ mcpName: "Lead.RouteLead", args: { rationale: "Enterprise SOURCE_SECRET" } }] });
