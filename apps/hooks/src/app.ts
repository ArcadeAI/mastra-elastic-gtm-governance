...
type Receipt = { operation_key: string; actor: string; action: string; lead_id: string; body: Inputs; completed_at: string };
...
export function createHooksApp(config: HooksConfig) {
  ...
  const leadToolkit = config.leadToolkit ?? "Lead", approvalsToolkit = config.approvalsToolkit ?? "Approvals";
  ...
  function safe(value: unknown, actor: string, tool: ToolRef = { toolkit: leadToolkit, name: "GetLead" }) {
  ...
  function normalizedTool(name: string): string {
    ...
    if (map?.route && name === map.route) return `${leadToolkit}.RouteLead`;
    if (map?.classify && name === map.classify) return `${leadToolkit}.ClassifyLead`;
    if (map?.requestApproval && name === map.requestApproval) return `${approvalsToolkit}.RequestApproval`;
    if (map?.decide && name === map.decide) return `${approvalsToolkit}.Decide`;
    return name;
  }
  ...
  function publicApproval(request: Approval) {
    return {
      ...
      resource_id: request.inputs.lead_id, required_clearance: request.required_clearance,
  ...
  function safeRunDisplay(value: unknown, actor: string) {
    ...
    return filterOutput(value, rules, subject(actor) ?? initial.subjects[0]!, { toolkit: leadToolkit, name: "GetLead" });
  }
  ...
  function safeTrace(value: unknown, actor: string): unknown[] { requireThat(Array.isArray(value), 400, "tool_calls must be an array."); return safeRunDisplay(value, actor) as unknown[]; }
  async function lead(path: string): Promise<Response> {
    ...
    catch { throw new HttpError(503, "Lead service could not be reached."); }
  }
  async function currentValue(id: string) {
    const response = await lead(`/internal/leads/${encodeURIComponent(id)}/value`);
    requireThat(response.ok, 503, "Authoritative value could not be read.");
    const result = object(await response.json());
    requireThat(result.lead_id === id && typeof result.estimated_acv === "number" && Number.isFinite(result.estimated_acv) && result.estimated_acv >= 0, 503, "Invalid authoritative value response.");
    return result.estimated_acv;
  }
  ...
  function actionBody(inputs: Inputs, name: string) { return name === "RouteLead" ? { estimated_acv: inputs.estimated_acv, owner_email: inputs.owner_email, rationale: inputs.rationale } : { disposition: inputs.disposition, rationale: inputs.rationale }; }
  function exactReceipt(saved: Receipt, inputs: Inputs, actor: string, tool: ToolRef) { return saved.operation_key === inputs.operation_key && saved.actor === actor && saved.lead_id === inputs.lead_id && saved.action === (tool.name === "RouteLead" ? "route" : "classify") && canonical(saved.body) === canonical(actionBody(inputs, tool.name)); }
  ...
  async function pre(raw: unknown) {
    ...
    const write = tool.toolkit === leadToolkit && ["RouteLead", "ClassifyLead"].includes(tool.name);
    let value: number | null = null;
    if (write) {
      if (typeof inputs.operation_key !== "string" || !keyPattern.test(inputs.operation_key) || typeof inputs.lead_id !== "string") return deny(actor, tool, "A valid operation_key and lead_id are required. Do not retry.", request.execution_id);
      ...
      if (existing) {
        if (!exactReceipt(existing, inputs, actor, tool)) return deny(actor, tool, "Operation key conflicts with a completed action. Do not retry.", request.execution_id);
      ...
      if (tool.name === "RouteLead") {
        value = await currentValue(inputs.lead_id);
        person = subject(actor); compiled = policy().compiled;
        if (!person || !stageAllows(actor, tool)) return deny(actor, tool, "This identity or stage cannot use this tool. Do not retry.", request.execution_id);
        const currentVisibility = resolveVisibility(person, [tool], compiled)[0]!;
        if (currentVisibility.decision.effect === "deny") return deny(actor, tool, currentVisibility.decision.reason, request.execution_id);
        if (inputs.estimated_acv !== value) return deny(actor, tool, "Asserted ACV differs from the authoritative value. Do not retry.", request.execution_id);
      }
    }
    let decision = evaluatePermission({ subject: person, tool, inputs: value === null ? inputs : { ...inputs, estimated_acv: value }, policy: compiled });
    if (decision.effect === "deny" && decision.rule_id === "authority-limit" && value !== null) {
      ...
      if (match) {
        ...
        const valid = grant.granted_by !== actor && grant.revoked_at === null && Date.parse(grant.expires_at) > clock() && match.value === value && approver?.attributes.can_approve === true && approver.clearance >= value && approval.status === "approved" && grant.resource_id === inputs.lead_id && (match.claimed_operation === inputs.operation_key || (match.claimed_operation === null && (grant.uses_remaining ?? 1) > 0));
        ...
        return deny(actor, tool, `This route exceeds your authority. Call ${approvalsToolkit}.RequestApproval with denial_id="${denial.id}" and justification="Explain why this exact action is needed". After approval retry the identical action.`, request.execution_id);
      }
    }
  ...
  function post(raw: unknown) {
    const request = PostHookRequest.parse(raw), actor = request.context.user_id ?? "", person = subject(actor);
    ...
    try {
      ...
      if (actor === config.verificationUserId && changed && request.success !== false) { const s = state(); s.verified_filter = true; store.put("state", "current", s); }
  ...
  function access(raw: unknown) {
    const request = AccessHookRequest.parse(raw), person = subject(request.user_id), { compiled } = policy();
  ...
  function requestApproval(body: Inputs) {
    const id = string(body.denial_id, "denial_id"), requester = string(body.requester_id, "requester_id"), justification = string(body.justification, "justification");
  ...
  async function decide(id: string, body: Inputs, token: string | null) {
    ...
    const value = await currentValue(string(before.inputs.lead_id, "lead_id"));
    return store.transaction(() => {
  ...
  function runs(id: string, operation: string, body: Inputs) {
    return store.transaction(() => {
      ...
      if (operation === "action") {
        ...
        requireThat([`${leadToolkit}.RouteLead`, `${leadToolkit}.ClassifyLead`].includes(normalizedTool(next.tool_name)), 400, "Action tool is not configured.");
        ...
        requireThat(run.status === "awaiting_snapshot" && run.tool_call_id === body.tool_call_id && run.request_id, 409, "No matching suspension is being persisted."); requireThat(getApproval(run.request_id).notification_status === "sent", 409, "Slack delivery is not confirmed.");
        ...
        const approval = getApproval(run.request_id); requireThat(approval.status === "approved" && approval.expires_at > clock(), 409, "Approval is pending, denied or expired.");
  ...
  async function handle(req: Request): Promise<unknown> {
    ...
    if (hookPath) authentication(req, config.hookSecret); else if (path.startsWith("/operator/")) authentication(req, config.operatorToken); else if (path.startsWith("/internal/approvals/") && req.method === "POST") authentication(req, config.approvalsToken); else authentication(req, config.webToken);
    ...
    if (operatorRun) {
      ...
      if (operatorRun[2] === "evidence" && req.method === "GET") {
        ...
        let action = null;
        if (run.action) {
          const inputs = run.action.arguments, name = normalizedTool(run.action.tool_name).endsWith(".RouteLead") ? "RouteLead" : "ClassifyLead";
          action = { operation_key: run.action.operation_key, tool_name: run.action.tool_name, arguments_hash: hash(inputs), receipt_binding_hash: hash({ actor: run.requester_user_id, action: name === "RouteLead" ? "route" : "classify", lead_id: inputs.lead_id, body: actionBody(inputs, name) }) };
        }
    ...
    if (path === "/operator/activate" && req.method === "POST") { const s = state(); requireThat(s.verified_denial && s.verified_filter, 409, "Verify denial and filtered output through the gateway before activation."); s.active = true; store.put("state", "current", s); return s; }
