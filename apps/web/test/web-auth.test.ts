...
beforeAll(async () => {
  ...
  const app = createWebApp({ runtime: () => createRuntime({ model: scriptedModel(["Supplied evidence only"]) }),
    session: () => ({ origin: base, idp, clientId: credentials.client_id, clientSecret: credentials.client_secret, secret: "test-session-secret-with-more-than-32-chars", emails, demoMode: true }),
...
test("APR1.R2 real OAuth session switches Dana to Riley while preserving pending Dana run", async () => {
  ...
  const verified = await browser.fetch(`${base}/auth/arcade/verify?flow_id=flow&user_id=${emails.dana}`);
