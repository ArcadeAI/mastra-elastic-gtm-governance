...
const stages = [
  { id: "supplied", label: "1 · Build your agent", hint: "Mastra", prompt: "What evidence supports qualifying Northwind, and what is missing?" },
  { id: "elastic", label: "2 · Connect evidence", hint: "Elastic through Arcade", prompt: "What evidence supports qualifying Northwind, and what is missing? Search Elastic and cite source IDs." },
  { id: "governed", label: "3 · Act with approval", hint: "Arcade + Slack", prompt: "Research LD-2291 and its Elastic account evidence. Route it to the configured sales manager if qualified. Follow the exact approval remediation if Arcade blocks the action." },
] as const;
...
export function Workshop({ approvalId }: { approvalId?: string }) {
  ...
  async function api(path: string, body?: unknown) {
    ...
    if (data.authorizationUrls?.length) setAuthUrls(data.authorizationUrls);
    return data;
  }
  ...
  async function submit() { const next = await api("/api/agent", { stage, message }); setResult(next); if (next.runId) rememberRun(next.runId); }
  ...
  return <main className="workshop-shell">
    <header className="workshop-header"><p className="eyebrow">MASTRA × ELASTIC × ARCADE</p><h1>Your lead agent, connected.</h1><p>Research the evidence. Propose the action. Get the right approval.</p></header>
    ...
    <div className="workspace-grid"><section className="workshop-card">
      ...
      {result?.status === "failed" && <p className="error-message" role="alert">The action did not complete. Review authorization or delivery details before starting another exercise.</p>}
      ...
      {authUrls.length > 0 && <div className="notice"><strong>Authorization needed</strong><p>Finish consent in the matching role, then retry the pending action.</p>{authUrls.map((url) => <p key={url}><a href={url} target="_blank" rel="noreferrer">Authorize with Arcade ↗</a></p>)}</div>}
    ...
    </section><aside className="workshop-card workflow-panel"><p className="eyebrow">THE CONNECTED WORKFLOW</p><ol><li>Mastra researches the lead</li><li>Elastic supplies the evidence</li><li>Arcade checks the action</li><li>Slack delivers the approval request</li><li>Riley approves; Dana’s agent continues</li></ol>
      ...
      {approval && <section className="approval-state"><h2>Human review</h2><p><strong>{approval.status}</strong> · Slack: {approval.notification_status}</p><p>{approval.requester_name} → {approval.approver_name}</p><code>{approval.tool_name}</code><pre>{JSON.stringify(approval.inputs, null, 2)}</pre>{approval.status === "pending" && <div className="button-row"><button disabled={busy || !session} onClick={() => perform(() => decide("approve"))}>Approve exact action</button><button className="secondary" disabled={busy || !session} onClick={() => perform(() => decide("deny"))}>Deny</button></div>}{approval.status === "approved" && runId && <button disabled={busy || !session || run?.status === "completed"} onClick={() => perform(async () => { setResult(await api(`/api/runs/${runId}/resume`, {})); await refresh(); })}>Resume Dana’s agent</button>}</section>}
