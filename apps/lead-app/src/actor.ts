...
/** Accept explicit service URLs as well as the existing bare-host configuration. */
function baseUrl(host: string): string {
  const local = /^(localhost|127\.0\.0\.1)(:|\/|$)/.test(host);
  const url = new URL(/^https?:\/\//.test(host) ? host : `${local ? "http" : "https"}://${host}`);
  if (!["http:", "https:"].includes(url.protocol) || url.username || url.password || url.search || url.hash) throw new Error("Invalid IdP service URL.");
